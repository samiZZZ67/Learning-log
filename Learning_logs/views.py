...
def  new_topic(request):
    ...
    if request.method != 'POST': 
        ...
        # POST data submitted; process data.
         ...
         if form.is_valid():
            topic_name = form.cleaned_data.get('text')
            if Topic.objects.filter(text__iexact=topic_name, owner=request.user).exists():
                # Do something if it exists (e.g., show an error)
                form.add_error('text', 'You already have a topic with this name.')
            else:
